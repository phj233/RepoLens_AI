...
struct HiddenFlutterMacContentView: NSViewControllerRepresentable {
...
struct RepoLensMacSidebarButton<Icon: View>: View {
  ...
  var body: some View {
    RepoLensNativeGlassPanel {
      HStack(spacing: 10) {
        ...
        RepoLensMacToastCloseButton(onClose: onClose)
      }
    }
...
struct RepoLensMacToastCloseButton: View {
  let onClose: () -> Void
...
struct RepoLensMacImagePreviewOverlay: View {
  let path: String
        ...
        VStack(alignment: .leading, spacing: 12) {
          ...
          if let image = NSImage(contentsOfFile: path) {
            Image(nsImage: image)
              .resizable()
              .scaledToFit()
              .frame(maxWidth: 980, maxHeight: 680)
